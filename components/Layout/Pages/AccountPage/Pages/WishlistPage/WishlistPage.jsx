...
export default function WishListPage() {
    const { wishList, addToCart, handleRemove, clearWishlist, fetchWishlist } = useMyContext();
    ...
    useEffect(() => {
        const initWishlist = async () => {
            try {
                ...
                console.log("line no. 20 - fetch wishlist error :", error);
            }
        };
        ...
        <AccountPage>
            <div className="flex flex-col items-center justify-center min-h-[60vh] px-6 text-center">
                <div className="relative mb-8">
                    ...
                    <div className="absolute -top-1 -right-1 w-6 h-6 bg-black rounded-full border-4 border-white flex items-center justify-center">
                ...
                <Link
                    href="/shop"
                    className="group flex items-center gap-3 bg-black text-white px-10 py-4 rounded-full hover:bg-neutral-800 transition-all duration-500 shadow-lg hover:shadow-xl"
                >
            ...
            <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-16">
                ...
                <header className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-4">
                    <div className="space-y-1">
                        <span className="text-[10px] uppercase tracking-[0.2em] text-gray-400 font-semibold">Your Selection</span>
                        <h1 className="text-4xl md:text-5xl font-serif text-gray-950 tracking-tighter">Wishlist</h1>
                    </div>
                    <div className="h-px flex-1 bg-gray-100 mx-8 hidden md:block mb-3" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-16">
                    {wishList.map((product) => (
                        <div key={product._id} className="group relative flex flex-col">
                            ...
                            <div className="relative aspect-[3/4] overflow-hidden bg-[#F9F9F9] rounded-[20px] transition-all duration-500">
                                ...
                                {/* Floating Remove Button (Glassmorphism) */}
                                <button
                                    onClick={() => handleRemove(product._id)}
                                    className="absolute top-4 right-4 z-20 p-3 bg-white/40 backdrop-blur-md text-gray-900 rounded-full border border-white/20 opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-red-500 hover:text-white transform hover:rotate-90"
                                    title="Remove"
                                >
                                    <FiTrash2 size={16} />
                                </button>
                                ...
                                <div className="absolute inset-x-0 bottom-0 p-6 z-10 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                                    <button
                                        ...
                                        className="w-full bg-black text-white py-4 rounded-xl text-xs font-bold uppercase tracking-widest shadow-2xl hover:bg-neutral-800 transition-colors flex items-center justify-center gap-2"
                                    >
                                        <FiPlus /> Add to Cart
                                    </button>
                                </div>
                            ...
                            <div className="mt-6 space-y-2">
                                <div className="flex justify-between items-start">
                                    <div className="max-w-[70%]">
                                        <h3 className="text-sm font-semibold text-gray-900 tracking-tight leading-tight group-hover:underline decoration-1 underline-offset-4 transition-all">
